...
class MT5Dataloader:
    def __init__(self, batch_size, data_dir, t5_model_dir):
        ...
        self.t5_tokenizer = PegasusTokenizer.from_pretrained(t5_model_dir, legacy=False)
    ...
    def collate_fn(self, batch):
        ...
        for text in batch:
            ...
            if len(text1_ids) != len(text2_ids) or len(text1_ids) not in [5, 7]:
                continue
            enc1_ids = self.prompt1 + text1_ids
            enc1_ids.append(self.t5_tokenizer.vocab[self.t5_tokenizer.eos_token])  # 添加结束id

            dec1_ids = copy.deepcopy(text2_ids)
            dec1_ids.insert(0, self.t5_config.decoder_start_token_id)

            y1 = copy.deepcopy(text2_ids)
            y1.append(self.t5_tokenizer.vocab[self.t5_tokenizer.eos_token])
            ...
            enc2_ids.append(self.t5_tokenizer.vocab[self.t5_tokenizer.eos_token])  # 添加结束id
            ...
            y2.append(self.t5_tokenizer.vocab[self.t5_tokenizer.eos_token])
        ...
        pad_token_id = self.t5_tokenizer.vocab[self.t5_tokenizer.pad_token]
        for i in range(len(batch_enc_len)):
            ...
            batch_enc_input[i].extend([pad_token_id] * enc_pad_len)
    ...
    def get_dataloader(self):
        ...
        return train_dataloader, test_dataloader, self.t5_tokenizer.vocab_size
    ...
    def data_processing(self) -> list[list]:
        ...
        for text in new_datas:
            final_datas.append(list(text))
            total_token.update(list(text))
        ...
        unk_token = set()
        vocab = self.t5_tokenizer.vocab.keys()
        for t in list(total_token):
            if t not in vocab:
                unk_token.update(t)
        print(unk_token)
        return final_datas
